...
GameObject::Pattern::Pattern(Color &color, std::string name, int radius, int boundRoundThickness)
    : GameObject(color), radius(radius), name(name), boundRoundThickness(boundRoundThickness)
{
    setupShape();
}
...
int GameObject::Pattern::getRadius() const // 取得半徑
{
    return radius;
}
...
void GameObject::Pattern::setupShape()
{
    ...
    sf::FloatRect rect = boundRound.getLocalBounds(); //原點定位到圓心
...
{
    GameObject::setColor(color);                      // 設定顏色
    boundRound.setFillColor(color.getLightColor());   // 內部填色
    boundRound.setOutlineColor(color.getDarkColor()); // 外框填色
}