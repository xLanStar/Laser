...
GameObject::ColorTile::ColorTile(sf::Vector2f position, sf::Font &font, Color &color, sf::SoundBuffer &soundBuffer, std::function<void()> onClick, int tileWidth, int tileHeight) : GameObject(color, position), text(position, 48, color, font, color.getName()), pantone(position, color, soundBuffer, onClick), tileWidth(tileWidth), tileHeight(tileHeight)
{
